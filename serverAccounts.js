 ...
 * @param collectiondrivers_shoppers_central: list of all drivers
 * @param resolve
                ...
                let resolveResponse =
                    ...
                    /rider/i.test(req.user_nature)
                        ...
                        : {
                              ...
                              IndexName: 'taxi_id',
                              KeyConditionExpression:
                                  'taxi_id = :val1, request_type = :val2',
                              ExpressionAttributeValues: {
                                  ':val1': req.user_fingerprint,
                                  ':val2': 'scheduled',
                              },
 ...
 * @param collectiondrivers_shoppers_central: list of all the drivers
 * @param resolve
    ...
    new Promise((res) => {
            ...
            .then((result) => {
                if (result.length > 0) {
                    ...
                    result.map((driver) => {
                        driver.cars_data.map((car) => {
                            if (
                                request.car_fingerprint === car.car_fingerprint
                            ) {
                                logger.info(car);
                                car_brand = car.car_brand;
                                ...
                                plate_number =
                                    plate_number[0] +
                                    ' ' +
                                    plate_number.substring(
                                        1,
                                        plate_number.length - 1
                                    ) +
                                    ' ' +
        ...
        (result) => {
            if (result !== false) {
                ...
                full_request_schema.car_details.plate_number =
                    result.plate_number;
                full_request_schema.car_details.car_picture =
                    result.car_picture;
                full_request_schema.car_details.taxi_number =
                    result.taxi_number;
                full_request_schema.car_details.vehicle_type =
                    result.vehicle_type;
                full_request_schema.car_details.verification_status =
                    'Verified'; //By default
                ...
                full_request_schema.numberOf_passengers =
                ...
                new Promise((res4) => {
                    ...
                    (e_travel_time) => {
                        if (e_travel_time !== false) {
                            //Found an eta
                            //Add eta
                            full_request_schema.estimated_travel_time =
                                e_travel_time;
                            //Cache the result
 ...
 * @param collectiondrivers_shoppers_central: collection of all the drivers
 ...
 * @param collectiondrivers_shoppers_central: the list of all the drivers.
                ...
                if (
                    ...
                    resultTransactionsReceived.map((transaction) => {
                        ...
                        if (
                            ...
                            receivedDataShot.total += parseFloat(
                                transaction.amount
                            );
            ...
            (receivedTransactionsData) => {
                ...
                new Promise((res) => {
                            ...
                            if (
                                ...
                                resultTransactions.map((transaction) => {
                                    if (
                                        ...
                                        //! Add
                                        detailsData.topedupAmount += parseFloat(
                                            transaction.amount
                                        );
                                    } else if (
                    ...
                    transaction
                                ...
                                let filterPaidRequests = /rider/i.test(
                                    user_type
                                )
                                    ? {
                                          client_id:
                                              requestObj.user_fingerprint,
                                ...
                                provideDataForCollection(
                                    ...
                                    .then((resultPaidRequests) => {
                                        ...
                                        if (
                                            ...
                                            let completedDataPromise =
                                                resultPaidRequests.map(
                                                    (paidRequests) => {
                                                        return new Promise(
                                                            (
                                                                partialResolver
        ...
        let parentPromises = walletBasicData.transactions_data.map(
            (transaction) => {
                return new Promise((resCompute) => {
                    ...
                    templateOne.transaction_nature =
                        transaction.transaction_nature;
                    ...
                    templateOne.total_rides = /ride/i.test(
                        transaction.transaction_nature
                    )
                        ? 1
                        : 0;
                    ...
                    templateOne.taxiconnect_commission =
                    ...
                    templateOne.driver_weekly_payout =
                        /weeklyPaidDriverAutomatic/i.test(
                            transaction.transaction_nature
                        )
                            ? transaction.amount
            ...
            .then(
                (resultBulkCompute) => {
                    if (
                        ...
                        let parentPromises2 = resultBulkCompute.map(
                            (weekData, index) => {
                                return new Promise((resComputePack) => {
                                    ...
                                    if (
                                        ...
                                        savedRecordOBJ.total_earning +=
                                            parseFloat(weekData.earning_amount); //Cash and wallet
                                        savedRecordOBJ.total_earning_wallet +=
                                            weekData.earning_amount_wallet; //Only wallet
                                        savedRecordOBJ.total_rides +=
                                            weekData.total_rides;
                                        savedRecordOBJ.total_deliveries +=
                                            weekData.total_deliveries;
                                        ...
                                        let weekTemplate = {
                                            ...
                                            total_earning: parseFloat(
                                                weekData.earning_amount
                                            ), //Cash and wallet included
                                            total_earning_wallet:
                                                weekData.earning_amount_wallet, //Without commission removal - wallet amount
                                        ...
                                        _GLOBAL_OBJECT.weeks_view[index] =
                                            weekTemplate;
                                        resComputePack(true);
                                    }
                                });
                            ...
                            .then(
                                (resultGlobalData) => {
                                    if (resultGlobalData.length > 0) {
                                        ...
                                        _GLOBAL_OBJECT.weeks_view.map(
                                            (weekData) => {
                                                totalEarnings +=
                                                    weekData.total_earning; //Money made
                                                totalDues +=
                                                    weekData.total_earning_due_to_driver +
                                                    weekData.total_earning_due_to_driver_cash; //Money due - WALLET AND CASH
                                                totalDues_wallet +=
                                                    weekData.total_earning_due_to_driver; //Only dues for the wallet
                                                totalPayouts +=
                                                    weekData.driver_weekly_payout; //Money already transaferred.
                                                totalComission +=
                                                    weekData.total_taxiconnect_commission; //Comission already transferred to US
                                            }
                                        );
                                        logger.info(`
...
function updateRiders_generalProfileInfos(
    ...
    if (requestData.infoToUpdate === 'name') {
        ...
        requestData.dataToUpdate = /^male$/i.test(
        ...
        if (requestData.direction === 'initChange') {
            ...
            if (requestData.dataToUpdate.length > 7) {
                ...
                resolve({
                    response: 'error',
                    flag: 'The phone number looks wrong.',
                });
            }
        ...
        try {
            if (isBase64(requestData.dataToUpdate, { mimeRequired: true })) {
                ...
                let tmpPicture_name = `${
                    requestData.user_fingerprint
                }_${Math.round(tmpDate.getTime())}.${formatImg}`;
                //...
                let regCleaner = new RegExp(
                    `data:image/${formatImg};base64,`,
                    'i'
                );
                    ...
                    function (err) {
                        ...
                        new Promise((resUploadPic_toS3) => {
                            ...
                            s3.upload(params, function (err, data) {
                                ...
                                logger.info(
                                    `Riders profile picture uploaded successfully.`
                                );
                        ...
                        try {
                            let freshObject = {
                                referral_fingerprint:
                                    itemReferral.referral_fingerprint,
                                taxi_number: itemReferral.taxi_number,
                                is_referralExpired:
                                    itemReferral.is_referralExpired,
                                ...
                                if (timeLeftRaw > 60) {
                                    ...
                                    if (timeLeftRaw > 0) {
                                        ...
                                        if (timeLeftRaw > 24) {
                                            ...
                                            //? Change in days
                                            timeLeftRaw =
                                                Math.round(timeLeftRaw);
                                            if (timeLeftRaw > 1) {
                                                timeLeftRaw = `${Math.round(
                                                    timeLeftRaw
                                                )} days`;
                                            } else {
                                                timeLeftRaw = `${Math.round(
            ...
            if (
                ...
                tripsData.map((trip) => {
                    ...
                    let tmpFare = /RIDE/i.test(trip.ride_mode)
                        ...
                        ? parseFloat(
                              trip.totals_request.service_fee.replace('N$', '')
                          ) +
                          parseFloat(
                              trip.totals_request.cash_pickup_fee.replace(
                                  'N$',
                                  ''
                              )
                          )
                        : parseFloat(
                          ...
                          parseFloat(
                              trip.totals_request.delivery_fee.replace('N$', '')
                          ); //basic trip fare
                    ...
                    //...
                    rating +=
                        trip.request_state_vars.rating_data.rating !==
                            undefined &&
                ...
                dynamo_find_query({
                    ...
                    .then((resltCheck) => {
                        if (resltCheck !== undefined && resltCheck.length > 0) {
                            ...
                            resolve({
                                response:
                                    'error_creating_account_alreadyExists',
                            });
                        } //? NEW ACCOUNT
                        else {
                            //Generate fp
                            let fpString = `${inputData.email}-${
                                inputData.first_name
                            }-${inputData.last_name}-${inputData.phone}-${
                                inputData.selected_industry
                        ...
                        dynamo_find_query({
                            ...
                            .then((companyData) => {
                                if (
                                    ...
                                    resolve({
                                        response:
                                            'error_logging_in_notFoundAccount',
                                    });
                                }
                            })
                            .catch((error) => {
                ...
                dynamo_find_query({
                    ...
                    .then((companyData) => {
                        if (
                            ...
                            //Company exists
                            let onlyDigitsPhone = inputData.phone
                                .replace('+', '')
        ...
        .then((resp) => {
            if (resp !== null && avoidCache === false) {
                ...
                try {
                    //Rehydrate
                    new Promise((resCompute) => {
                        execGetWalletSummaryForDeliveryCorps(
                            company_fp,
                            resCompute
                        );
                    })
...
function execGetWalletSummaryForDeliveryCorps(company_fp, resolve) {
    ...
    dynamo_find_query({
        ...
        IndexName: 'company_fp',
        KeyConditionExpression:
            'compnay_fp = :val1, transaction_nature = :val2',
        ExpressionAttributeValues: {
            ':val1': company_fp,
...
/**
 * @function sendSMSAny
 * Responsible to send SMS in any direction while keeping the security standards based on quotas.
 * based on the daily limit that the number has ~ 10SMS per day.
 * @param req: the request data containing the user's phone number : ATTACH THE _id if HAS AN ACCOUNT
 * @param resolve
 * @param service: ride or courier
 */
function sendSMSAny({ req, resolve, service }) {
    const DAILY_THRESHOLD = parseInt(process.env.DAILY_SMS_THRESHOLD_PER_USER);
 ...
 */
exports.processCourierDrivers_application = async (req) => {
    try {
        let {
            phone,
            nature_driver,
        ...
        if (driver.count > 0)
            return { response: 'error_duplicate_application' };

        //New application
        //? Parse the json string data
        personal_details = JSON.parse(personal_details);
        extensions = JSON.parse(extensions);
        vehicle_details = JSON.parse(vehicle_details);

        //? Make file names
        let driverPhotoName = `driverPhoto_${uuidv4()}.${
            extensions.driver_photo
        }`;
        let vehiclePhoto = `vehiclePhoto${uuidv4()}.${
            extensions.vehicle_photo
        }`;
        let licensePhoto = `licensePhoto${uuidv4()}.${
            extensions.license_photo
        }`;
        let idPhoto = `idPhoto${uuidv4()}.${extensions.id_photo}`;

        const uploadedDriverPicture = await uploadBase64ToS3(
            driver_photo,
            `${process.env.S3_main_bucket}/drivers/profile_pictures`,
            driverPhotoName
        );
        const uploadedVehiclePicture = await uploadBase64ToS3(
            vehicle_photo,
            `${process.env.S3_main_bucket}/drivers/vehicle_pictures`,
            vehiclePhoto
        );
        const uploadedLicensePicture = await uploadBase64ToS3(
            license_photo,
            `${process.env.S3_main_bucket}/drivers/license_pictures`,
            licensePhoto
        );
        const uploadedIdPicture = await uploadBase64ToS3(
            id_photo,
            `${process.env.S3_main_bucket}/drivers/id_pictures`,
            idPhoto
        );

        if (
            !uploadedDriverPicture ||
            !uploadedVehiclePicture ||
            !uploadedLicensePicture ||
            !uploadedIdPicture
        )
            return {
                response: 'error_reformation_remote',
                error: 'Failed to upload one or more pictures',
            };
        ...
        await DriversApplications.create({
            ...
            documents: {
                driver_photo: uploadedDriverPicture,
                vehicle_photo: uploadedVehiclePicture,
                license_photo: uploadedLicensePicture,
                id_photo: uploadedIdPicture,
            },
            accepted_conditions_details: {
                did_accept_terms:
                    did_accept_terms && did_accept_terms === 'true',
                did_certify_data_veracity:
                    did_certify_data_veracity &&
                    did_certify_data_veracity === 'true',
            },
        });
        ...
        logger.error(error);
        return { response: 'error_failed_integrity_checks' };
    }
};
